...
int main(int argc, char **argv) {
  ros::init(argc, argv, "test_periodic_cloud");

  // create the action client
  // true causes the client to spin its own thread
  actionlib::SimpleActionClient<vision::BTAction> ac(
      "periodic_cloud_aggregator", true);
  vision::BTResult node_result;
  ...
  while (command != 3) {
    ...
    ROS_INFO(
        "Send a command: 1:start the action | 2:stop the action | 3:exit the "
        "program");
    std::cin >> command;
